    ...
    <div className="bg-jasmine-white dark:bg-slate-950 pb-24">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-12 grid grid-cols-1 lg:grid-cols-3 gap-12">
        ...
        <div className="lg:col-span-1">
          <div className="sticky top-28 bg-white dark:bg-slate-900 rounded-3xl shadow-xl border border-slate-200 dark:border-slate-800 overflow-hidden">
            <div className="bg-mediterranean-blue p-6 text-white text-center">
              {tour.price > 0 ? (
                <>
                  <p className="text-white/80 text-sm uppercase tracking-wider font-semibold mb-1">From</p>
                  <div className="flex justify-center items-end gap-1">
                    <span className="text-5xl font-bold font-heading">€{tour.price}</span>
                    <span className="mb-1 text-white/80">/ person</span>
                  </div>
                </>
              ) : (
                <div className="flex justify-center items-center h-16">
                  <span className="text-3xl font-bold font-heading">Custom Price</span>
                </div>
              )}
            </div>
            ...
            <div className="p-6 space-y-6">
              <div className="space-y-4">
                <div className="flex items-center gap-3 text-sm text-slate-600 dark:text-slate-400">
                  <MapPin className="w-5 h-5 text-terracotta-warmth" />
                  <span><strong>Meeting Point:</strong> {tour.meetingPoint}</span>
                </div>
                <div className="flex items-center gap-3 text-sm text-slate-600 dark:text-slate-400">
                  <Clock className="w-5 h-5 text-terracotta-warmth" />
                  <span><strong>Departures:</strong> {tour.departureTimes.join(", ")}</span>
                </div>
                <div className="flex items-center gap-3 text-sm text-slate-600 dark:text-slate-400">
                  <CalendarDays className="w-5 h-5 text-terracotta-warmth" />
                  <span>{tour.cancellation}</span>
                </div>
              </div>

              <TourBookingForm tourName={tour.name} basePrice={tour.price} />
            </div>
          </div>
        </div>
